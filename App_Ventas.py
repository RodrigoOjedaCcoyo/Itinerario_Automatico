...
# --- LÓGICA DE PDF (PLATAFORMA INDEPENDIENTE) ---
import pdfkit
...
def generar_pdf_web(tours, pasajero, fechas, categoria, modo, vendedor, celular, cover_img, title_1, title_2, info_precios):
    ...
    # Función auxiliar para obtener imágenes locales
    def obtener_imagenes_tour(nombre_carpeta):
        base_path = Path('assets/img/tours') / nombre_carpeta
        
        if not base_path.exists():
            base_path = Path('assets/img/tours/general')
        
        imagenes = []
        if base_path.exists():
            for f in base_path.iterdir():
                if f.suffix.lower() in ['.png', '.jpg', '.jpeg', '.webp']:
                    # Usar rutas relativas para compatibilidad con pdfkit/wkhtmltopdf
    ...
    # Imagen de portada (Dinámica - Ruta relativa)
    uri_portada = cover_img
