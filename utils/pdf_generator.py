...
def get_image_as_base64(path):
    ...
    try:
        p = Path(path)
        if not p.is_absolute(): p = BASE_DIR / path
...
def generate_pdf(itinerary_data, output_filename=OUTPUT_FILENAME):
    ...
    # Cargar CSS como texto para inyección directa vía Playwright (Más seguro)
    css_content = ""
    if CSS_FILE.exists():
    ...
    # Script Playwright que INYECTA el CSS directamente en el navegador
    script_content = f'''
import asyncio
import sys
...
async def main():
    try:
        async with async_playwright() as p:
            ...
            # INYECTAR CSS DESDE EL SCRIPT (SOLUCIÓN DEFINITIVA)
            await page.add_style_tag(content=r"""{css_content}""")
            
            # Forzar estilos de iconos
