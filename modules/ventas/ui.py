    ...
    with col2:
        ...
        if not st.session_state.itinerario:
            ...
            for t in st.session_state.itinerario:
                ...
                # Se redondea cada tour individualmente (math.ceil) para que el total
                # coincida con la suma de los precios por tour mostrados en pantalla.
                total_nac += (math.ceil(c_nac * f_m_t) * c_ad_nac)
                total_nac += (math.ceil(c_nac_es * f_m_t) * c_es_nac)
                total_nac += (math.ceil(c_nac_pc * f_m_t) * c_pc_nac)
                total_nac += (math.ceil(c_nac_ni * f_m_t) * c_ni_nac)

                total_ext += (math.ceil(c_ext * f_m_t) * c_ad_ext)
                total_ext += (math.ceil(c_ext_es * f_m_t) * c_es_ext)
                total_ext += (math.ceil(c_ext_pc * f_m_t) * c_pc_ext)
                total_ext += (math.ceil(c_ext_ni * f_m_t) * c_ni_ext)

                total_can += (math.ceil(c_can * f_m_t) * c_ad_can)
                total_can += (math.ceil(c_can_es * f_m_t) * c_es_can)
                total_can += (math.ceil(c_can_pc * f_m_t) * c_pc_can)
                total_can += (math.ceil(c_can_ni * f_m_t) * c_ni_can)

                # Acumulación para Totales "Antes" (con margen global 'factor_a')
                total_nac_a += (math.ceil(c_nac * f_a) * c_ad_nac)
                total_nac_a += (math.ceil(c_nac_es * f_a) * c_es_nac)
                total_nac_a += (math.ceil(c_nac_pc * f_a) * c_pc_nac)
                total_nac_a += (math.ceil(c_nac_ni * f_a) * c_ni_nac)

                total_ext_a += (math.ceil(c_ext * f_a) * c_ad_ext)
                total_ext_a += (math.ceil(c_ext_es * f_a) * c_es_ext)
                total_ext_a += (math.ceil(c_ext_pc * f_a) * c_pc_ext)
                total_ext_a += (math.ceil(c_ext_ni * f_a) * c_ni_ext)

                total_can_a += (math.ceil(c_can * f_a) * c_ad_can)
                total_can_a += (math.ceil(c_can_es * f_a) * c_es_can)
                total_can_a += (math.ceil(c_can_pc * f_a) * c_pc_can)
                total_can_a += (math.ceil(c_can_ni * f_a) * c_ni_can)

                # Acumulación Marginada para Detalle por Pasajero
                cost_margined_nac_ad += math.ceil(c_nac * f_m_t)
                cost_margined_nac_es += math.ceil(c_nac_es * f_m_t)
                cost_margined_nac_pc += math.ceil(c_nac_pc * f_m_t)
                cost_margined_nac_ni += math.ceil(c_nac_ni * f_m_t)

                cost_margined_ext_ad += math.ceil(c_ext * f_m_t)
                cost_margined_ext_es += math.ceil(c_ext_es * f_m_t)
                cost_margined_ext_pc += math.ceil(c_ext_pc * f_m_t)
                cost_margined_ext_ni += math.ceil(c_ext_ni * f_m_t)

                cost_margined_can_ad += math.ceil(c_can * f_m_t)
                cost_margined_can_es += math.ceil(c_can_es * f_m_t)
            ...
            idioma_pdf = st.selectbox(
                ...
                ["Español", "English", "Français", "Deutsch", "Português", "Italiano", "日本語 (Japonés)", "Русский (Ruso)"],
            ...
            with st.expander("📝 Páginas Adicionales de Texto Libres (Ej. Trekkings, Equipaje)", expanded=False):
                ...
                with col_ex1:
                    ...
                    p1_texto = st.text_area("Contenido", value="", key="pax_p1_txt", height=200, max_chars=1300, placeholder="- Zapatos de trekking altos\n- Pastillas para el soroche")
                with col_ex2:
            ...
            gen_pdf_clicked = False
            tab_pdf, tab_prev = st.tabs(["📄 Generar PDF Final", "👁️ Vista Previa en Vivo"])
            with tab_pdf:
                if st.button("🔥 GENERAR ITINERARIO PDF", use_container_width=True):
                    gen_pdf_clicked = True
            with tab_prev:
                if st.button("🔄 Generar / Actualizar Vista Previa", type="primary", use_container_width=True):
                    preview_clicked = True

            if gen_pdf_clicked or preview_clicked:
                if celular and st.session_state.itinerario:
                    with st.spinner("Procesando vista previa..." if preview_clicked else "Generando PDF con Edge..."):
                        base_dir = os.getcwd()
                        ...
                        try:
                            ...
                            full_itinerary_data = {
                                ...
                                'labels': translated_data.get('labels') if idioma_pdf != "Español" else {
                            ...
                            if gen_pdf_clicked:
                                ...
                                st.info("Renderizando Vista Previa...")
                                from utils.pdf_generator import render_html_preview
                                import streamlit.components.v1 as components
                                try:
                                    html_str, _ = render_html_preview(full_itinerary_data, is_preview=True)
                                    with tab_prev:
